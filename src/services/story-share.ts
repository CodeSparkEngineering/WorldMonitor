...
export function generateStoryDeepLink(
  ...
  return `https://geonexus.app/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  twitter: (data: StoryData) =>
    ...
    `\nGenerated by @codesparkengineering`,
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via GeoNexus - Open source geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - CodeSparkEngineering`)}`,
