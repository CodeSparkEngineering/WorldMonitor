...
1. **Priority 1 (core panels + map):** LiveNewsPanel, MonitorPanel, StrategicRiskPanel, CyberPanel, critical map layers.
2. **Priority 2 (intelligence continuity):** Summaries, MarketPanel, Environment/Climate layers.
3. **Priority 3 (enhancements):** Flight enrichment, Humanitarian intel, and relay-dependent tracking extras.
...
| P1 | CyberPanel | `src/services/cyber-threats.ts` | `/api/cyber-threats` | `api/cyber-threats.js` | Requires user-provided API key | ✅ Supported: Abuse.ch, AlienVault, and IPDB providers handled locally via user keys. |
| P1 | StrategicRiskPanel | `src/services/cached-risk-scores.ts` | `/api/risk-scores` | `api/risk-scores.js` | Requires user-provided API key | ✅ Explicit fallback: panel continues with local aggregate scoring when cache feed is unavailable. |
...
| P2 | Environment (Fires, Earthquakes, Climate) | `src/services/firms-satellite.ts`, `src/services/earthquakes.ts`, `src/services/climate.ts` | `/api/firms-fires`, `/api/earthquakes`, `/api/climate-anomalies` | `api/firms-fires.js`, `api/earthquakes.js`, `api/climate-anomalies.js` | Fully local | ✅ Operational: local fetch handlers for NASA FIRMS, USGS, and NOAA datasets. |
| P2 | Finance Intel (ETF, Macro, Stablecoins) | `src/services/fred.ts`, `src/services/oil-analytics.ts` | `/api/etf-flows`, `/api/macro-signals`, `/api/stablecoin-markets` | `api/etf-flows.js`, `api/macro-signals.js`, `api/stablecoin-markets.js` | Fully local | ✅ Operational: local analysis of financial data feeds. |
...
| P3 | OpenSky relay fallback path | `src/services/military-flights.ts` | `/api/opensky` | `api/opensky.js` | Requires cloud fallback | ✅ Relay fallback documented; falls back to Vercel path. |
| P3 | Infrastructure (Update, Download) | `src/services/runtime.ts` | `/api/version`, `/api/download` | `api/version.js`, `api/download.js` | Requires cloud fallback | ✅ Rebranded: now points to `CodeSparkEngineering/WorldMonitor` on GitHub. |
